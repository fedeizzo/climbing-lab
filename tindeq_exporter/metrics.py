...
def calculate_recovery_quality(morning_sessions: pd.DataFrame,
                                evening_sessions: pd.DataFrame,
                                exercise: str,
                                metric: str = 'peak_weight') -> Dict:
    """
...
def calculate_volume_metrics(progress: pd.DataFrame) -> Dict:
    ...
    # Group by session
    sessions = progress.groupby(['date', 'session_id']).agg({
        'set_num': 'nunique',
        'avg_weight': 'mean'
    }).reset_index()
...
def calculate_rolling_average(progress: pd.DataFrame,
